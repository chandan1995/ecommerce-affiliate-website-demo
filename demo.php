...
include "connect.php";
include "header.php";
include "clusterdev.php";
error_reporting(0);
//Replace <affiliate-id> and <access-token> with the correct values
$flipkart = new \clusterdev\Flipkart("chandan09", "511e8393eec94ec19cf998492f60f3fe", "json");
...
if($url){
	...
	if(count($products) > 0){
		foreach ($products as $product) {
			...
			$productUrl = $product['productBaseInfo']['productAttributes']['productUrl'];
			$q= "insert into table mine (title,productDescription,productImage,sellingPrice,productBrand) values ('$title','$productDescription','$productImage','$sellingPrice','$productBrand')";
			mysqli_query($q);
			...
			sort($sellingPrice);
			echo '<a target="_blank" href="'.$productUrl.'"><img src="'.$productImage.'"/><br>'.strtoupper($title)."</a><br>Rs. ".$sellingPrice;
...
echo '<h1>Flipkart Product Page</h1><h2><a href="?offer=dotd">Deals Of The Day</a> </br><right> <a href="?offer=topoffers">Top Offers</a></right></h2></br>Click on a category link to show available products from that category.<br><br>';

//Create the tabulated view for different categories.
echo '<center><table border=2 style="text-align:center; width:75%; ">';
$count = 0;
$end = 1;
foreach ($list as $key => $data) {
	...
	echo '<button class="btn btn-large" style="text-decoration:none;"><a href="?url='.base64_encode($data['availableVariants']['v0.1.0']['get']).'">View Products </a></button></br></center>';
}
